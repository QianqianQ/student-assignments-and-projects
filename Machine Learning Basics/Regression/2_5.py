...
def gradient_descent(x,y,w,alpha,n,iterations):
    risk=[]
    xTrans = x.transpose()
    for i in range(0, iterations):
        hypothesis = np.dot(x, w)
        bias = hypothesis - y
        cost = (np.dot(bias.transpose(),bias))[0][0] / n
        risk.append(cost)
        # avg gradient per example
        gradient = 2*(np.dot(xTrans, bias)) / n
        # update
        w = w - alpha * gradient
    return w,risk
...
if __name__ == "__main__":
    ...
    for i in range(len(alpha)):
        w_t, risk_t = gradient_descent(x_matrix,y_matrix,origin_w,alpha[i],len(y),len(iterations))
        risk.append(risk_t)
        w.append(w_t)
