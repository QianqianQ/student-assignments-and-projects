...
Start by familiarizing yourself with the classes ``ExpressionTree`` and
    ``ETNode``, which were included in the exercise package.
    ``ExpressionTree`` is a binary tree, which can be used to convert
    postfix expressions into infix format.
    The conversion takes place by first building a binary tree from the operators and
    operands of the postfix expression.
    After the tree is complete, the infix version of the original expression can be
    obtained by traversing the nodes of the tree using an inorder traversal.

    The class ``ExpressionTree`` is almost ready, but it lacks the method
    used to traverse the nodes in correct order.
    Your task is to implement the required functionality into the method
    ``_visit_inorder`` so that it returns an iterable (a list will do also) of the
    values of the tree nodes in inorder.
    Remember also to add parentheses at correct positions.
    Even though the parentheses would be trivial, for example ``((1*2)+3)``,
    they should still be added for every operator.
    This is of course a bit redundant, but it simplifies the task.
...
class ExpressionTree:
    ...
    def construct_from_postix(self, expression):
    ...
    def _visit_inorder(self, starting_node=None):
        """Return an iterable yielding the values of the visited nodes while adding parentheses to retain operator precedence."""
        list=[]
